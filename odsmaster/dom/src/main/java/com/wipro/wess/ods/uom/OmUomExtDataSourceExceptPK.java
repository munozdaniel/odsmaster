package com.wipro.wess.ods.uom;

import java.util.StringTokenizer;

import com.wipro.wess.ods.upload.ProvisioningConstants;
import com.wipro.wess.ods.utils.OmUtil;

// Generated Jan 7, 2014 12:15:31 PM by Hibernate Tools 3.2.2.GA

/**
 * OmUomExtDataSourceExceptId generated by hbm2java
 */
public class OmUomExtDataSourceExceptPK implements java.io.Serializable {

    /**
     * 
     */
    private static final long serialVersionUID = -4083073952165485483L;

    public MExtDataSourceInfoPK extDataSourceId;

    public MeasurementEntityPK measureEntityId;

    public MeasurementPK measureId;

    public OmUomExtDataSourceExceptPK() {
    }

    public OmUomExtDataSourceExceptPK(MExtDataSourceInfoPK extDataSourceId, MeasurementEntityPK measureEntityId,
            MeasurementPK measureId) {
        this.extDataSourceId = extDataSourceId;
        this.measureEntityId = measureEntityId;
        this.measureId = measureId;
    }

    public OmUomExtDataSourceExceptPK(String key) {
        StringTokenizer token = new StringTokenizer(OmUtil.decode(key), ProvisioningConstants.DOMAIN_OBJ_SEPERATOR);
        this.extDataSourceId = new MExtDataSourceInfoPK(token.nextToken());
        this.measureEntityId = new MeasurementEntityPK(token.nextToken());
        this.measureId = new MeasurementPK(token.nextToken());
    }

    @Override
    public String toString() {
        return OmUtil.encode("" + this.extDataSourceId.toString() + ProvisioningConstants.DOMAIN_OBJ_SEPERATOR
                + this.measureEntityId.toString() + ProvisioningConstants.DOMAIN_OBJ_SEPERATOR
                + this.measureId.toString());
    }

    public MExtDataSourceInfoPK getExtDataSourceId() {
        return this.extDataSourceId;
    }

    public void setExtDataSourceId(MExtDataSourceInfoPK extDataSourceId) {
        this.extDataSourceId = extDataSourceId;
    }

    public MeasurementEntityPK getMeasureEntityId() {
        return this.measureEntityId;
    }

    public void setMeasureEntityId(MeasurementEntityPK measureEntityId) {
        this.measureEntityId = measureEntityId;
    }

    public MeasurementPK getMeasureId() {
        return this.measureId;
    }

    public void setMeasureId(MeasurementPK measureId) {
        this.measureId = measureId;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + ((extDataSourceId == null) ? 0 : extDataSourceId.hashCode());
        result = prime * result + ((measureEntityId == null) ? 0 : measureEntityId.hashCode());
        result = prime * result + ((measureId == null) ? 0 : measureId.hashCode());
        return result;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (obj == null)
            return false;
        if (getClass() != obj.getClass())
            return false;
        OmUomExtDataSourceExceptPK other = (OmUomExtDataSourceExceptPK) obj;
        if (extDataSourceId == null) {
            if (other.extDataSourceId != null)
                return false;
        } else if (!extDataSourceId.equals(other.extDataSourceId))
            return false;
        if (measureEntityId == null) {
            if (other.measureEntityId != null)
                return false;
        } else if (!measureEntityId.equals(other.measureEntityId))
            return false;
        if (measureId == null) {
            if (other.measureId != null)
                return false;
        } else if (!measureId.equals(other.measureId))
            return false;
        return true;
    }

}
